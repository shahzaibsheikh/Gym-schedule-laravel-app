                  ...
                  <div class="py-6">
                     <div class="flex gap-6 justify-between">
                        <div>
                           ...
                           <p class="text-2xl font-bold text-purple-700">{{ $class->classType->ct_name }}</p>
                           <p class="text-sm">{{ $class->instructor->name }}</p>
                           <p class="mt-2">{{ $class->classType->ct_description }}</p>
                           <span class="text-slate-600 text-sm">{{ $class->classType->ct_minutes }} minutes</span>
                        </div>
                     ...
                     <div class="mt-1 text-right">
                        <form action="{{ route('booking.store') }}" method="POST"  role = "form">
                           @csrf
                           <input type="hidden" name="scheduled_class_sc_id" value="{{$class->sc_id}}">
